<?php	
	...
	{			
		...
		if(!empty($_POST['produto_id_add']))
		{			
			##verifica se produto é válido
			$produto = GoCURL(array('id_cliente'=>$_SESSION['id_cliente'], 'produto_id'=>$_POST['produto_id_add']), 'produtos/validar');
			
			if(!$produto['success'])
			{
				$mensagem = $produto['message'];				
				$return = array('sucesso'=>false, 'mensagem'=>$mensagem);
			}		
			else
			{			
				$carrinho = '';
				...
				$dados = array('id_cliente'=>$_SESSION['id_cliente'], 
							   'produto_id'=>$_POST['produto_id_add'], 
							   'add'=>'true', 
							   'carrinho' => $carrinho);				
				...
				if(!empty($_SESSION['pedido']))
				{
					$return = array('sucesso'=>true, 'mensagem'=>'produto adci com sucesso');
				}
				else
				{
					$return = array('sucesso'=>false, 'mensagem'=>'Ocorreu um erro na geração do seu pedido. Tente novamente');	
				}
		...
		else if(!empty($_POST['pedido_loading']))
		{
			$host 				 = $_POST['host'];
			$percentual_definido = $_POST['percentual_definido'];
			require_once('../../web/pedido.php');	
			exit;		
		}
		else
		{
